...
from ext2TeV.spectra import Spectra
from ext2TeV.utils import logparabola
...
class Source(object):
	...
	def set_ebl_interpolator(self):
		try:
			eblmodel = self.spectra.eblmodel
		except:
			eblmodel = Table.read(
	...
	def broadband_sed(self, dataset, model=None, figure_path=''):
		print(' * {}'.format(dataset['common_name']))
		model_orig = str(model)
		if model is not None:
			model_modifiers = model.split(",")[1:]
		else:
			model_modifiers = []
		model = model.split(",")[0]
		...
		if matched is not None:
			...
			# modifiers
			for modif in model_modifiers:
				gammamodel[model][modif.split("=")[0]] = float(modif.split("=")[1])
		...
		for model in ["pwl", "lp", "plec"]:
			...
			yfit = self.spectra.flexible_model(xfit * 1e6, model=gammamodel[model]) * fitatten
			sedplot.plot(xfit, yfit, label=model_orig)
		...
		plt.savefig("{}/FermiAndVHE_{}_{}.png".format(
			figure_path, vhe['name'].replace(" ", ""),
			model_orig.replace(",","_")),
